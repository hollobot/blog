...
![image-20250916124407588](./assets/image-20250916124407588.png)
...
- 避免key冲突
- 方便管理
- 更节省内存： key是string类型，底层编码包含`int`、`embstr` 和 `raw` 三种。embstr在小于44字节使用，采用连续内存空间，内存占用更小。当字节数大于44字节时，会转为raw模式存储，在raw模式下，内存空间不是连续的，而是采用一个指针指向了另外一段内存空间，在这段空间里存储SDS内容，这样空间不连续，访问的时候性能也就会收到影响，还有可能产生内存碎片

![image-20250916124521304](./assets/image-20250916124521304.png)
...
![image-20250916124631605](./assets/image-20250916124631605.png)
...
![image-20250916130511262](./assets/image-20250916130511262.png)
...
![image-20250916131713167](./assets/image-20250916131713167.png)
