...
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | --------------- | ------------------------------------------------------- |
| 类加载阶段   | 首次使用类的静态资源（访问静态变量、调用静态方法、new 对象等） | 1. 初始化类的静态变量（如 `static int count;`）2. 执行静态代码块（`static {}`） | 全局仅 1 次     | 先父类加载 + 父类静态块 → 再子类加载 + 子类静态块       |
| 对象创建阶段 | 执行 `new 类名()` 显式创建对象                               | 1. 初始化非静态普通属性（`类型 变量名`）2. 执行非静态代码块（`{}，实例代码块`）3. 执行构造器（`类名()`） | 每次 new 均执行 | 先父类非静态块 → 父类构造器 → 子类非静态块 → 子类构造器 |
