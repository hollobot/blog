...
|         类型         |              核心特点              |      适用场景       |      核心风险       |
| :------------------: | :--------------------------------: | :-----------------: | :-----------------: |
|   FixedThreadPool    |  线程数固定，无空闲线程，无界队列  |    稳定长期任务     |   队列堆积 → OOM    |
|   CachedThreadPool   | 核心线程 0，线程数无上限，60s 回收 | 短期 / 突发轻量任务 | 线程过多 → CPU 满载 |
| SingleThreadExecutor |      单线程串行执行，无界队列      |  需顺序执行的任务   |   队列堆积 → OOM    |
| ScheduledThreadPool  | 核心线程固定，支持定时 / 周期任务  |  定时 / 周期性任务  | 线程过多 → CPU 满载 |

#### 二、自定义线程池（生产首选）

直接用 `ThreadPoolExecutor`，自定义核心 / 最大线程数、有界队列、拒绝策略，规避预定义线程池的无界 / 无上限风险。

#### 总结

1. 预定义 4 种：固定、缓存、单线程、定时（测试用）；
2. 自定义线程池：生产环境首选，参数可控、风险低；
3. 核心：避免无界队列 / 线程数无上限。
...
- 这段代码**不能保证 t1 → t2 → t3 顺序打印**，三个线程打印顺序完全随机。
- `join()` 写在所有 `start()` 之后，作用只是**让主线程等全部子线程跑完再结束**，**不会约束子线程之间的执行先后**；

```java
t1.start(); // 仅告诉JVM就绪，进入线程调度队列，不一定立刻运行
t2.start();
t3.start();

t1.join(); // 主线程阻塞，等待t1执行完毕
t2.join(); // 主线程阻塞，等待t2执行完毕
t3.join(); // 主线程阻塞，等待t3执行完毕
```
...
Semaphore 就像**有限数量的许可证**，线程要执行任务必须先 “拿证”，执行完再 “还证”：

- 初始化时指定许可证数量（比如 3 个）；
- 线程调用 `acquire()` 拿证：有证就拿走，没证就排队等；
- 线程调用 `release()` 还证：把证还回去，唤醒排队的线程。

核心作用：**控制同时访问某个资源的线程数量**（限流）。

#### 二、极简示例（控制最多 2 个线程同时执行）

```java
import java.util.concurrent.Semaphore;

public class SemaphoreDemo {
    // 初始化 2 个许可证（最多 2 个线程同时执行）
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            new Thread(() -> {
                try {
                    semaphore.acquire(); // 拿证：没证就等
                    System.out.println(Thread.currentThread().getName() + " 开始执行");
                    Thread.sleep(1000); // 模拟任务执行
                } catch (InterruptedException e) {} 
                finally {
                    semaphore.release(); // 还证：必须释放，否则许可证会耗尽
...
**效果**：始终只有 2 个线程在执行，其余 3 个排队，等前序线程还证后再执行。

#### 三、关键细节（简短版）

- 两种模式：
  - 公平模式：按线程排队顺序拿证；
  - 非公平模式（默认）：线程直接抢，可能插队（效率更高）。
- 常用场景：
  - 限流（如接口最多允许 100 个并发）；
  - 控制资源访问（如最多 5 个线程读写同一个文件）。

#### 总结

1. 信号量核心是「许可证机制」，控制同时访问资源的线程数；
2. `acquire()` 拿证、`release()` 还证，必须成对使用；
3. 核心用途：并发限流、资源隔离。
