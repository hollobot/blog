...
- **第二步：遍历桶中元素（链表或红黑树）**
  ...
  - 若桶是链表：
    ...
    - 先比较节点哈希值与新 key 的哈希值是否一致；
    - 若哈希值一致，再通过`==`（引用相同）或`equals()`（内容相同）判断 key 是否匹配。
    - 若匹配到相同 key：直接覆盖该节点的 value，流程终止；
    - 若遍历至链表尾部仍无匹配：在链表**尾部插入新节点**（JDK 1.8 采用尾插法，解决 JDK 1.7 头插法在扩容时可能引发的死链问题）；插入后检查链表长度，若≥8 则触发链表转红黑树的结构转换。
...
桶内元素可能以**链表**（元素数量≤8）或**红黑树**（元素数量 > 8）形式存储，需逐一匹配键：
...
- 调用红黑树的查找方法（如getTreeNode()），通过树的特性高效查找节点：
  - 比较节点的哈希值和键的哈希值，结合`compareTo()`或`equals()`方法匹配目标`key`。
...
key → 计算哈希值（hash(key)）→ 计算桶索引 → 桶是否为空？
   → 否 → 桶内是链表？→ 遍历链表匹配key → 返回value/null
        → 桶内是红黑树？→ 红黑树查找匹配key → 返回value/null
   → 是 → 返回null
```
