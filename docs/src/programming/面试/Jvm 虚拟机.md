...
- 内容
  ...
  - 符号引用：编译时无法确定实际内存地址，仅以符号形式存在的引用，包括：
...
## 26. JVM 工作原理（通俗 + 核心架构 + 运行流程）

JVM（Java 虚拟机）是**跨平台运行环境**，核心作用：**一次编译，到处运行**，把 `.class` 字节码翻译成当前操作系统可执行指令。

#### 一、整体核心流程

`Java源码(.java)` → **编译** → `字节码(.class)` → **JVM 加载 + 执行** → 操作系统运行

1. **前端编译**：`javac` 把 `.java` 编译为平台无关的 **Class 字节码**
2. **JVM 运行**：JVM 读取 Class 文件，经过**类加载、内存分配、执行、垃圾回收**四大环节运行程序
...
负责把 `.class` 文件加载到内存，分**3 大阶段**：

1. **加载**：读取 class 文件，生成 `Class` 对象
2. **链接**：验证合法性 → 分配静态内存 → 符号引用转直接引用
3. **初始化**：执行静态代码块、给静态变量赋初始值

**双亲委派机制**（核心规则）：

子类加载器先委托父类加载，父类无法加载再由子类加载，**防止核心类被篡改**。

**2. 运行时数据区（JVM 内存模型）**

程序运行时的数据存储区域，分**线程私有**和**线程共享**：

**线程私有（随线程创建 / 销毁）**

- **程序计数器**：记录当前执行字节码行号，唯一无 OOM 的区域
- **虚拟机栈（Java 栈）**：存**栈帧**（局部变量、方法出口等），方法调用 / 结束对应栈帧入栈 / 出栈
- **本地方法栈**：执行 `native` 本地方法（C/C++ 实现）

**线程共享（全局共用）**

- **堆（Heap）**：**存放所有对象实例、数组**，GC 主要回收区域
- **方法区（元空间）**：存类结构、常量、静态变量、即时编译代码

**3. 执行引擎**

真正执行字节码的组件，主流两种执行方式：

1. **解释器**：逐行翻译字节码，启动快、执行慢
2. **即时编译器（JIT）**：把**热点代码**编译成本地机器码，执行更快（主流 JDK 默认开启）

#### 三、垃圾回收 GC（堆内存核心机制）

堆中对象不再被引用时，GC 自动回收内存，避免内存泄漏。

1. **判断对象存活**：主流 **可达性分析**（从根对象遍历，不可达则回收）

2. **常用回收算法**：标记清除、复制、标记整理、分代收集
...
1. `javac` 编译 `.java` → `.class` 字节码
2. 类加载器按**双亲委派**加载 class 到内存
3. 线程创建，虚拟机栈、程序计数器初始化
4. 执行引擎**解释 + JIT**执行字节码，对象分配到堆
5. 后台 GC 定时清理堆中无效对象
6. 线程结束，私有内存自动释放
...
JVM 就是**一套统一的虚拟计算机**，屏蔽 Windows/Linux/Mac 系统差异，负责**加载类、管理内存、执行代码、自动垃圾回收**，实现 Java 跨平台运行。
...
GC（Garbage Collection）垃圾收集器是**JVM 自动管理堆内存、回收无引用对象**的后台线程，核心目标：自动识别死亡对象、释放堆内存，避免手动内存泄漏，解决 C/C++ 手动 malloc/free 的内存管理痛点。 判定对象死亡两种算法：

1. 引用计数法：对象被引用 + 1，引用消失 - 1，0 则回收；缺陷：循环引用无法回收。
2. 可达性分析（HotSpot 默认）：从 GC Roots 向下遍历，不可到达的对象判定为垃圾。 GC Roots：栈局部变量、静态变量、本地方法栈 JNI 引用、Class 对象、锁对象等。

#### 二、分代收集理论（所有收集器设计基础）

堆划分为**新生代 + 老年代**，不同区域对象存活特性不同，采用不同回收算法：

1. 新生代：对象生命周期短、大量对象快速死亡 → 

   复制算法

   - Eden 区（80%）、Survivor0/Survivor1（各 10%）
   - Minor GC：Eden 满触发，存活对象复制到空闲 Survivor，清空 Eden

2. 老年代：对象长期存活、存活率高 → 

   标记 - 清除 / 标记 - 整理

   - Major GC：仅回收老年代；Full GC：整堆（新生代 + 老年代 + 元空间）全部回收，STW 极长，尽量避免。

3. 分代优势：针对不同存活特性使用最优算法，提升 GC 效率、降低 STW。

##### 三种基础回收算法对比

| 算法     | 原理                             | 优点               | 缺点                         | 使用场景                   |
| -------- | -------------------------------- | ------------------ | ---------------------------- | -------------------------- |
| 标记清除 | 标记垃圾→统一清除                | 简单，无需移动对象 | 内存碎片，分配大对象容易 OOM | CMS 老年代                 |
| 复制     | 存活对象复制到新空间，原空间清空 | 无碎片，分配快     | 需要额外内存开销             | 新生代                     |
| 标记整理 | 标记垃圾，存活对象向一端压缩     | 无碎片             | STW 时间更长                 | Serial Old、G1 老年代、ZGC |

#### 三、HotSpot 主流垃圾收集器分类（面试必考）

按发展顺序、使用场景分为**串行、并行、并发、低延迟**四大类：

##### 1. Serial 串行收集器（单线程）

- Serial New：新生代复制算法，单线程 GC，全程 STW
- Serial Old：老年代标记整理，单线程
- 适用：客户端、单核小内存程序，嵌入式设备
- 缺点：多核 CPU 只能用一个线程，STW 时间长

##### 2. Parallel 并行收集器（多线程，追求吞吐量）

多 GC 线程并行工作，**目标：最大化吞吐量**（用户运行时间 / 总运行时间），牺牲停顿时间。

1. Parallel Scavenge（新生代）：复制算法，自适应调节新生代大小
2. Parallel Old（老年代）：标记整理

- 默认 JDK8 服务器模式收集器
- 适用：后台计算、批处理任务，不追求低延迟

##### 3. CMS 并发收集器（Concurrent Mark Sweep，并发标记清除）

第一款**并发低延迟收集器**，目标降低 STW 停顿，老年代专用，搭配 Parallel New 新生代。

##### CMS 四大阶段（重点）

1. 初始标记（STW）：仅标记 GC Roots 直接关联对象，极短停顿
2. 并发标记（无 STW）：用户线程和 GC 线程同时运行，遍历全部对象
3. 重新标记（STW）：修正并发阶段用户线程产生的浮动垃圾，停顿较短
4. 并发清除（无 STW）：同步清除垃圾，不 STW

##### CMS 缺陷（面试高频）

1. 使用标记清除，产生大量内存碎片，老年代空间不足触发 Full GC
2. 并发阶段占用 CPU，并发失败（Concurrent Mode Failure）：GC 速度赶不上对象创建，退化为 Serial Old Full GC
3. 浮动垃圾：并发清除阶段新产生的垃圾本次无法回收，只能下次 GC
4. JDK9 开始废弃，被 G1 替代

##### 4. G1（Garbage-First，JDK7 推出，JDK9 默认）

**区域化分代收集器**，不再严格区分新生代老年代，堆拆分为多个大小相等的 Region（2MB~32MB）。 核心思想：优先回收垃圾最多的 Region，减少整体停顿，兼顾吞吐量与低延迟。

1. Region 分类：Eden、Survivor、Old、Humongous（超大对象直接放，避免复制开销）
2. GC 类型：
   - Young GC：回收 Eden Region
   - Mixed GC：回收全部新生代 + 部分垃圾最多的老年代 Region（替代 CMS 的 Full GC）
3. 优势：
   - 可自定义最大停顿时间 `-XX:MaxGCPauseMillis`
   - 无大块内存碎片，Mixed GC 自带整理
   - 超大对象 Humongous 区域优化
4. 不足：多 Region 维护内存开销高于 CMS；极端低延迟不如 ZGC

##### 5. ZGC（Z Garbage Collector，JDK11 实验，JDK15 正式）

**超低延迟收集器**，几乎无 STW，停顿时间控制在 10ms 以内，TB 级大堆也稳定。 核心技术：读屏障、染色指针、内存重定位

1. 全程几乎并发，只有短暂初始标记、根扫描 STW
2. 不使用分代，统一 Region 布局
3. 优势：超大堆、低延迟在线服务（微服务、高并发网关）首选
4. 缺点：CPU 占用略高，吞吐量略低于 G1

##### 6. Shenandoah GC（OpenJDK，RedHat）

和 ZGC 定位一致，超低延迟，通过转发指针实现并发整理，相比 ZGC 兼容性更好，商用场景使用少于 ZGC。

#### 四、收集器搭配关系（面试画图考点）

收集器新生代、老年代必须配对使用，不能随意组合：

1. Serial New + Serial Old
2. Serial New + CMS Old
3. Parallel Scavenge + Parallel Old
4. Parallel New + CMS Old
5. G1、ZGC、Shenandoah：自身完整收集器，无需配对

#### 五、收集器选型面试标准答案

1. 小型客户端、单核嵌入式：Serial
2. 后台批处理、大数据计算、追求高吞吐量：Parallel 并行收集器
3. 中小型微服务、追求平衡延迟与吞吐量：G1（JDK9 + 默认）
4. 高并发在线业务、大堆、严格低延迟要求：ZGC
5. 老旧 JDK8 在线服务：CMS（不推荐，碎片问题多，建议升级 G1/ZGC）

#### 六、高频延伸面试问题（附带简答）

1. G1 和 CMS 区别？ CMS 按分代回收，标记清除有碎片；G1 按 Region 回收，可控制停顿，Mixed GC 整理碎片，无并发失败问题。
2. ZGC 为什么停顿极短？ 利用染色指针区分对象状态，移动对象全程并发，仅 GC Roots 扫描短暂 STW，堆再定位并发完成。
3. Full GC 触发场景？ 老年代空间不足、元空间溢出、CMS 并发失败、System.gc ()、堆内存泄漏、分配超大 Humongous 无空间。
4. STW 是什么？ Stop-The-World，GC 执行时暂停全部用户业务线程，所有收集器都存在 STW，只是时长不同。
5. 浮动垃圾是什么？ 并发 GC 阶段用户线程新生成的垃圾，当前 GC 无法处理，只能等待下一轮回收。

#### 七、简洁口语化总结（面试口述精简版）

JVM 垃圾收集器用于自动回收堆中无引用对象，基于分代理论设计，新生代复制、老年代标记清除 / 整理。主流收集器分为串行 Serial、高吞吐 Parallel、低延迟 CMS、平衡型 G1、极致低延迟 ZGC。Serial 适合单核小程序，Parallel 适合批计算，G1 是 JDK9 默认均衡选择，线上高并发低延迟需求推荐 ZGC。GC 都会产生 STW，各收集器核心差异在于 STW 时长、吞吐量、内存碎片处理方式，业务需根据延迟 / 吞吐量需求选型。
...
当 JVM 执行垃圾回收时，**所有 Java 用户业务线程全部暂停**，只有 GC 线程在工作，用户代码完全停止运行，这个暂停阶段就叫 STW。

不管哪种垃圾收集器（Serial / Parallel / CMS / G1 / ZGC）**都一定会有 STW**，区别只在于：停顿时间长短、STW 发生次数多少。

#### 3. 为什么必须 STW？

GC 需要标记存活对象、移动对象、清理垃圾，如果用户线程同时还在修改对象引用、创建对象，对象引用关系会持续变化，GC 标记结果就会错乱，出现漏标、错标，导致误回收还在用的对象。 所以必须暂停业务线程，保证 GC 标记阶段引用图稳定。

#### 4. 各收集器 STW 对比（面试重点）

1. **Serial / Parallel** 全程 STW，整个 GC 期间业务全部卡死，停顿时间最长。
2. **CMS** 只有「初始标记、重新标记」两段短暂 STW，并发标记、并发清除阶段用户线程正常跑。
3. **G1** Young GC 全程 STW；Mixed GC 只有初始标记、重新标记短暂停顿，整体可控，可通过参数限制最大停顿时间。
4. **ZGC / Shenandoah** 只有扫描 GC Roots 极短时间 STW，**停顿基本控制在 10ms 以内**，超大堆也不会拉长停顿，低延迟线上首选。

#### 5. 业务危害

线上接口超时、订单丢失、监控告警、微服务调用雪崩，高并发系统要尽量减少 STW 时长与频率。

#### 6. 口语简答（面试直接背）

STW 即 Stop-The-World，GC 运行时暂停所有业务线程，防止对象引用变动干扰垃圾标记。所有收集器都存在 STW，只是停顿长短不同；我们调优 GC 的核心目标就是减少 STW 时间，降低线上卡顿。