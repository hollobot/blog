...
2. **解析并改写 SQL**
   - 插件先获取原始 SQL（如 `SELECT * FROM user`），根据数据库类型（MySQL、Oracle 等）自动拼接分页语法（如 MySQL 加 `LIMIT ?`，Oracle 嵌套 `ROWNUM` 子查询）。
   - 同时，自动生成一个 “查询总条数” 的 SQL（如 `SELECT COUNT(*) FROM (原始SQL)`），用于获取总记录数（计算总页数）。
3. **执行分页查询**
   - 先执行改写后的分页 SQL，获取当前页的数据。
   - 再执行总条数 SQL，得到总记录数。
