...
## 60. MySQL 慢 SQL 排查与优化？

整体排查优化流程：**定位慢 SQL → 分析执行计划 → 定位根因 → 针对性优化 → 效果验证**

#### 一、第一步：定位慢 SQL

先找到具体是哪些 SQL 执行慢，是排查的前提。

**1. 慢查询日志（最常用）**

MySQL 自带的日志，专门记录执行时间超过阈值的 SQL。
...
- 日志分析工具：
  ...
  - `mysqldumpslow`：MySQL 自带，可按访问次数、耗时排序
  - `pt-query-digest`：Percona 工具，分析更详细，可生成统计报告

**2. 实时排查正在执行的慢 SQL**

```sql
-- 查看当前所有运行中的线程，State字段可看出是否在锁等待、文件排序等
SHOW processlist;
-- 查看完整SQL语句
SHOW FULL processlist;
```
...
- `information_schema`：查看锁等待、事务执行情况
- `performance_schema`：更细粒度的性能统计，可定位 IO、锁、阶段耗时
- 企业级监控：Prometheus+Grafana、SkyWalking 等 APM 工具，可线上实时告警慢 SQL

#### 二、第二步：用 explain 分析执行计划 

找到慢 SQL 后，用`explain + SQL语句`查看 MySQL 的执行计划，判断索引使用、扫描行数、排序方式等核心信息。

**重点字段解读（面试高频）**

|      字段       |                     核心含义 & 面试重点                      |
| :-------------: | :----------------------------------------------------------: |
|     **id**      |  SQL 执行顺序：id 值越大越先执行；id 相同，从上到下顺序执行  |
| **select_type** | 查询类型：- `SIMPLE`：简单查询，无子查询 /union- `PRIMARY`：主查询，最外层查询- `SUBQUERY`：where 后的子查询- `DERIVED`：派生表，from 后的子查询- `UNION`：union 后的第二个及之后的查询 |
|    **type**     | 访问类型，**性能从优到劣排序**：`system > const > eq_ref > ref > range > index > ALL`面试红线：出现`ALL`代表全表扫描，必须优化；`index`是全索引扫描，也需关注；日常至少要达到`range`级别，最优是`ref`/`eq_ref` |
|  possible_keys  |                  可能用到的索引（候选索引）                  |
|     **key**     |             实际使用的索引；为`NULL`代表没走索引             |
|     key_len     | 索引使用的字节数，可判断联合索引命中了几列（值越大，命中的索引列越多） |
|    **rows**     |             预估需要扫描的行数，数值越大性能越差             |
|    **Extra**    | 额外信息，**面试最高频考点**：- `Using index`：覆盖索引，无需回表，性能极好- `Using where`：存储引擎层过滤后，Server 层再过滤- `Using filesort`：文件排序，无法利用索引排序，需优化- `Using temporary`：使用临时表存中间结果（常见于 group by、order by），性能损耗大- `Using join buffer`：关联字段无索引，用了连接缓冲区 |
...
- 未建立合适索引，导致全表扫描
- 索引失效：触发了索引失效的场景
- 索引选错：MySQL 优化器选错了索引（统计信息不准）
- 回表过多：普通索引查询后，频繁回主键索引查数据

**2. SQL 语句写法问题**

- `select *` 查询无用字段，增加 IO 和回表概率
- 深分页问题：`limit 1000000, 10` 需扫描前 100 万行
- 索引列上做函数运算、算术运算
- `like '%xxx'` 左模糊查询，无法命中 B + 树索引
- 隐式类型转换（比如字符串字段用数字查询）
- 多表 join 关联字段无索引，或 join 表数量过多
- `or` 连接的条件中存在非索引列

**3. 数据与表结构问题**

- 单表数据量过大（千万级以上），未分库分表
- 字段类型设计不合理，字段过大、冗余字段多
- 冷热数据未分离，历史数据占用大量空间

**4. 数据库与锁问题**

- 长事务、锁等待导致 SQL 执行阻塞
- 行锁升级为表锁（比如未命中索引的 update）
- 缓冲池（innodb_buffer_pool）配置过小，频繁磁盘 IO

#### 四、针对性优化方案

**1. 索引优化（最核心、性价比最高）**

- **遵循最左前缀原则**：联合索引按区分度从高到低排列，查询条件从最左列开始匹配
- **使用覆盖索引**：让查询字段都包含在索引中，避免回表（Extra 出现 Using index）
- 消除索引失效：
  - 禁止在索引列上使用函数、计算、类型转换
  - 左模糊查询改用全文索引或 ES
  - 用`union`拆分`or`中无索引的条件
  - 避免`!=`、`not in`、`is not null` 等导致索引失效的操作
- 删除冗余、重复、低区分度的无效索引，减少维护成本
...
- Join 优化：小表驱动大表，关联字段必须建索引，控制 join 表数量（建议≤3 张）

- 无需去重时用`UNION ALL`代替`UNION`，省去排序去重开销

- 大批量操作分批执行，避免长事务和锁表

**3. 表结构与数据优化**

- 选择合适的数据类型：用小类型存小数据（比如 tinyint 代替 int），字段尽量设`NOT NULL`
- 垂直拆分：大字段、低频字段拆分到单独的表
- 水平拆分：单表超千万级后，按业务维度分库分表（如按用户 id 哈希、按时间范围）
- 历史数据归档：冷数据迁移到历史库 / 数仓，主库只保留热数据

**4. 数据库配置优化**

- `innodb_buffer_pool_size`：设置为物理内存的 50%~70%，减少磁盘 IO
- 调整`sort_buffer_size`、`join_buffer_size`，优化排序和关联缓冲区
- 合理设置事务隔离级别，RC 级别下可减少锁持有时间
- 开启`MRR`、`ICP`等索引优化特性

**5. 架构层面优化**

- **读写分离**：主库写、从库读，分摊读压力
- **引入缓存**：热点数据用 Redis 缓存，减少直接查 MySQL
- **复杂查询迁移**：全文检索用 ES，统计报表走数仓 / OLAP 引擎，不直接打在线库

#### 五、优化后验证

1. 重新执行`EXPLAIN`，确认 type、key、Extra 是否符合预期
2. 实际执行 SQL，查看执行时间、扫描行数是否下降
3. 观察慢查询日志，确认该 SQL 不再出现
4. 压测验证：确认高并发下性能稳定，无新的慢 SQL 产生



## 61. MySQL 服务器 CPU 突然压力暴增，怎么排查优化？

整体思路：**先紧急止损 → 由外到内定位根因 → 针对性优化验证**，核心覆盖 sql 索引问题与缓存失效两大高频诱因。

#### 一、快速排查流程（由外到内定位）

#### **1. 系统层：确认是 mysql 进程导致**

服务器执行 `top` 命令：

- 确认 `mysqld` 进程占用 cpu 占比最高
- 区分 cpu 分项：
  - `us` 高：sql 计算、扫描、排序等用户态开销，对应 sql / 索引问题
  - `sy` 高：线程上下文切换频繁，对应连接风暴、线程堆积
  - `wa` 高：本质是磁盘 io 瓶颈，非纯 cpu 问题

**2. 流量侧：快速排查缓存失效诱因**

通过监控核对两个核心指标：

- mysql 侧：qps、活跃连接数是否突发式暴涨，且以读请求为主
- 缓存侧：redis 命中率是否骤降、请求漏检量是否暴涨，或 redis 实例是否出现故障 / 重启
- 特征匹配：单条 sql 执行速度正常、索引无异常，但并发量远超平日，基本可判定为缓存失效引发的流量穿透

**3. 数据库层：抓取现场异常线程与 sql**

```sql
-- 查看所有实时运行线程，重点看time、state、info三列
show full processlist;
```

重点关注异常 state：

- `sorting result`：文件排序，cpu 消耗极高
- `copying to tmp table on disk`：磁盘临时表，计算 + io 双开销
- `sending data`：大量数据扫描传输，多为全表扫描
- `waiting for table metadata lock`：元数据锁等待，连接堆积

```sql
-- 统计总耗时top10的sql（需performance_schema开启，5.7+默认开启）
...
- 索引缺失或失效：新上线 sql 未建索引、触发索引失效规则（左模糊、索引列运算、隐式转换），高并发全表扫描打满 cpu
- 排序分组无索引：`order by`/`group by` 字段不匹配索引，触发内存 / 磁盘排序
- 低效写法：深分页大偏移量、多表无索引 join、大批量无索引更新删除
- 优化器选错索引：表统计信息过期，执行计划异常

**2. 缓存失效引发流量洪峰（突发场景高频）**

- **缓存雪崩**：大量 key 同时集中过期、redis 集群宕机 / 重启清空，全量读请求直接穿透到 mysql，qps 陡增数倍
- **缓存击穿**：单个超高热度 key 过期瞬间，海量并发同时回源查库，集中打同一条 sql
- **缓存穿透**：大量查询不存在的数据，持续绕过缓存直接打库，长期推高 cpu

**3. 其他辅助根因**

- 连接风暴：流量突增 + 客户端重试，连接数打满，上下文切换开销剧增
- 锁堆积：长事务持有锁不释放，查询排队阻塞，线程持续累加
- 定时任务：高峰期执行备份、表优化、全量统计等重 cpu 操作

#### 三、紧急止损手段

1. **清理异常线程**：找到 processlist 中耗时最长的慢 sql、长事务，执行 `kill 线程id` 快速释放 cpu
2. **缓存侧应急修复**：redis 故障切备用集群；集中过期则批量延长热点 key 有效期；快速回灌热点缓存数据
3. **业务限流降级**：网关层、服务层开启限流，非核心接口临时降级，拦截超额请求
4. **流量分摊**：读写分离架构下，将读流量切换到从库，分摊主库 cpu 压力
5. **连接兜底**：临时调小应用连接池最大连接数，避免 mysql 连接溢出彻底不可用

#### 四、长效优化方案

**1. sql 与索引优化**

- 为高频查询建立合适索引，遵循最左前缀原则，优先使用覆盖索引避免回表
- 优化 order by/group by 逻辑，匹配索引顺序，消除 filesort 与临时表
- 深分页改用书签式分页，控制 join 表数量（≤3 张），关联字段必须建索引
- 建立 sql 上线审核机制，所有上线 sql 必须经过 explain 校验，禁止无索引 sql 上线
- 开启慢查询日志，设置合理阈值，定期巡检优化慢 sql

**2. 缓存体系优化（防流量穿透）**

- 防雪崩：缓存过期时间增加随机偏移量；搭建 redis 高可用集群；重启后提前预热热点数据；引入本地多级缓存兜底
- 防击穿：热点 key 设置永不过期，后台异步更新；加分布式互斥锁，同一时间仅一个请求回源
- 防穿透：接入布隆过滤器拦截非法 key；对不存在的数据缓存短时效空值
- 接入熔断降级组件，数据库压力过高时自动熔断保护

**3. 架构与运维优化**

- 读写分离、冷热数据分离，单表数据量过大时做分库分表
- 报表、统计类复杂查询迁移至数仓 /olap 引擎，不占用在线库 cpu
- 监控 cpu 使用率、qps、连接数、缓存命中率等核心指标，设置分级告警
- 大表 ddl、备份、统计任务统一放在业务低峰期执行
