...
2. **面向切面（AOP）**：将日志、事务等通用功能抽离，不侵入业务代码，便于统一维护；
...
- 原理：运行时通过动态代理织入切面逻辑。
  - 若目标类实现接口：用 **JDK 动态代理**，生成接口代理类，代理类中嵌入切面代码（如日志、事务）。
  - 若目标类无接口：用 **CGLIB 代理**，生成目标类的子类，重写方法并嵌入切面逻辑。
...
1. **读配置**：容器启动时，加载配置信息（注解或 XML），确定要管理的类和规则。
2. **记定义**：扫描指定路径，识别带`@Component`、`@Bean`等的类，将其信息（类名、依赖等）记录为 “Bean 定义” 并注册。
3. **创对象**：按注册的定义，反射创建 Bean 实例，自动注入依赖，完成初始化后放入缓存（单例 Bean），供后续使用。
